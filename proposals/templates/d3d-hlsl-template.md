...
> This template is a guideline for documenting a feature with both HLSL and D3D
> surface. These require HLSL, DXIL, and D3D API changes and thereby if not
> hardware support, then at least driver support.
> 
> The section titles are meant to be retained in the final document, but the
> descriptions are to outline what should be in those sections and should be
> replaced by the feature-specific text. Not all sections may be required for
> all features.
>
...
> following first:
>
> 1. Do not spend time writing the "Detailed design" until the feature has
>    progressed to the **Draft** state.
> 2. Remove "draft: true" from the front matter once the feature is no longer in
>    draft.
...
_The detailed design is not required until the feature is moving from **Draft**
to **Review**._
...
* Are there new D3D API functions, structures, or enums introduced by this
  feature?
* What validation can be done in the debug layer for this feature?

### DDI Changes

* What changes to the DDI are required to support this feature?

### WARP Support

* What changes, if any, are required to the WARP software rasterizer to support
  this feature?

### PIX Support

* What changes, if any, are required to the PIX tool to support this feature?

### Other Tooling Impact

* What changes, if any, are required to other tooling to support this feature?
...
* Newly-introduced errors that cause existing shaders to newly produce errors
  ...
  * Changes that break previously valid shaders due to changes in what the
    compiler accepts related to this feature.
...
* If there are changes that will change how existing shaders compile, what
  transition support will we provide?
  * New compilation failures should have a clear error message and ideally a
    FIXIT
  ...
  * Errors that are produced for previously valid shader code would give ample
    notice to developers that the change is coming and might involve rollout
    stages
...
trouble from those who might want to suggest feedback or additional features
that might build on this on. Even variations on the chosen solution can be
interesting.
